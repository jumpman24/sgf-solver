...
from solver import init_model

model = init_model()


class TsumegoNode:
    def __init__(self, board: TsumegoBoard, turn, winner, terminal):
        self.board = board
        self.turn = turn
        self.winner = winner
    ...
    def find_children(self):
        ...
        predicted = model.predict([[self.board.board]])

    def find_random_child(self):
        """Random successor of this board state (for more efficient simulation)"""
        if self.terminal:
            return None  # the position is clear
    ...
    def find_winner(self):
        ...
        return self.turn if is_solved else not self.turn

    def make_move(self, x, y):
        board = self.board.move_and_copy(x, y)
        turn = not self.turn
        winner = self.find_winner()
        is_terminal = (winner is not None)
        return TsumegoNode(board, turn, winner, is_terminal)

    def __eq__(node1, node2):
        return node1.board.board == node2.board.board
